...
namespace vcpkg::Commands::Import
{
    ...
    static void check_is_directory(const Checks::line_info& linfo, const fs::path& dirpath)
    {
        Checks::check_exit(linfo, fs::is_directory(dirpath), "The path %s is not a directory", dirpath.string());
    }
